...
**Новые сделки (полный дамп):** `services.deal_params_5m.build_full_entry_context(d5)` пишет в context_json снимок уровня prompt_entry:
...
- `bars_count`, `kb_news_impact`, `session_phase` (из market_session), `estimated_upside_pct_day`, `suggested_take_profit_price`
...
- **Запись при BUY:** `scripts/send_sndk_signal_cron.py` вызывает `build_full_entry_context(d5)` и передаёт результат в `record_entry(..., entry_context=...)`.
