...
**Обновлено:** 2026-06-05 (срез по репозиторию `lse`, prod VM, `crontab/lse-docker.crontab`).
...
|-------------|-----------|-----------------|
| **Unified ML retrain (L1)** | Registry 8 контуров, dispatcher `15 */6`, 5 active refresh scripts, `ml_contours_status.json` | `3ad797b`, [ML_UNIFIED_RETRAIN_FRAMEWORK.md](ML_UNIFIED_RETRAIN_FRAMEWORK.md) |
...
|-------------|--------|---------------|-----------|
| **Документация** | Канон + rollout + archive stubs | — | 0 ✅ |
| **L1 watermark game5m** | SELL `ts` (closed round-trips) | — | 1.1 ✅ |
| **Nightly cron dedup** | `23:47` dispatcher `--slot nightly` | Deploy crontab на VM | 1.3 ✅ |
| **Legacy game5m train** | `23:40` datasets only | Train via dispatcher | 1.2 ✅ |
| **Registry-only контуры** | — (все 8 active) | Deploy + weekly_full cron | 2 ✅ |
| **L3 resolve на prod** | mirror | 3–7 дней без divergence → enable | 3 |
| **Recovery D4b** | Не внедрён | go/no-go по D4a stats | 3 |
...
3. **Recovery отдельный nightly cron** — в consolidation plan (фаза 2).
