...
flowchart TD
    ...
    F[knowledge_base] --> G[Автоматическая синхронизация]
    G --> B
    
    H[Исторические сделки] --> I[Извлечение паттернов]
    I --> B
```
...
```mermaid
flowchart TD
    A[AnalystAgent.get_decision] --> B[Технический анализ]
