...
### 2.2. Нерешённый контур: «ранний» выход и overnight (зафиксировано как пробел)

**Что наблюдаем.** Часть закрытий идёт как **`TIME_EXIT_EARLY`** (в UI часто подписано «ранний de-risk»), по факту у нас в проде это преимущественно ветка **`stale_reversal`** в `should_close_position`: позиция долго не ведёт себя как «быстрый» GAME-long (возраст, просадка относительно порога, слабый импульс, слабое решение). Отдельно пользовательский сценарий **«вошли в предыдущий RTH-день, обвал/гэп в следующий»** **не покрывается** отдельным предиктивным правилом: **5m-логика не страхует overnight-разрыв** между сессиями так же, как внутридневную просадку. **Премаркет** в текущей реализации даёт контекст для **входа** и осторожности у **открытия RTH** (`get_decision_5m`), плюс **exit-guard** в первые минуты после 9:30 (`GAME_5M_EXIT_GUARD_FIRST_MINUTES`), но **не** служит автоматическим «по утреннему аукциону решить: держать вчерашний long или выйти до обвала».

**Почему это трудно «доделать» сразу.** Любое смягчение `stale_reversal` (реже резать) увеличивает риск **глубже сидеть** в деградировавшем лонге; ужесточение — больше кейсов **«можно было подождать»** после факта. Контрфакт «могли бы подождать» по **`TIME_EXIT_EARLY`** считается в **`trade_effectiveness_analyzer`**: блок **`time_exit_early_review`** (post-exit MFE/MAE по 5m, whipsaw), но **числовые предложения** по `GAME_5M_STALE_REVERSAL_*` / `EARLY_DERISK_*` попадают в LLM/`auto_config_override` только когда в отчёте достаточно строк с OHLC и не срабатывает флаг **`insufficient_data_for_ml`** (см. `docs/TRADE_EFFECTIVENESS_ANALYZER.md`). **Новости / event-слой** для предсказания overnight-просадки у нас в **ранней фазе** — на него пока **нельзя** опираться как на стабильный автоматический гейт выхода без отдельной валидации.

**Статус:** зафиксировано как **открытая задача продукта/риска** (не баг отдельной строки кода): явный **overnight / cross-day risk policy** и связка **premarket → удержание уже открытой позиции** — **вне** текущего минимального контракта GAME_5M.

### 2.3. Пер-тикерно «поднять порог удержания» для части тикеров: что уже есть и кто «советует»

**Уже в коде (ручная настройка `config.env`).**

- **`GAME_5M_STALE_REVERSAL_MIN_AGE_MINUTES_<TICKER>`** — для тикера `TICKER` переопределяется только **минимальный возраст** до срабатывания `stale_reversal` (см. `_game_5m_stale_reversal_exit_params` в `services/game_5m.py`). Остальные поля ветки (`MAX_PNL_PCT`, `MOMENTUM_BELOW`) пока **глобальные** для всех тикеров.
- **`GAME_5M_MAX_POSITION_DAYS_<TICKER>`** и **`GAME_5M_TAKE_PROFIT_PCT_<TICKER>`** — пер-тикерные лимиты дней и cap тейка; префиксы подхватываются веб-редактором `/parameters` автоматически (см. `config_loader._GAME_5M_TICKER_KEY_PREFIXES`).

**Кто «советует» пороги (инструменты, не магия).**

1. **`GET /api/analyzer`** / **`scripts/export_analyzer_report.py`** / **`scripts/run_analyzer_docker.sh`** на сервере — сводка, **`game5m_hanger_v2_review`**, **`time_exit_early_review`** (когда есть OHLC), **`practical_parameter_suggestions`**, опционально LLM.
2. **Узкий срез:** **`scripts/analyze_trades_focused.py`** с **`--tickers`** / **`--trade-ids`** и **`--llm`** — LLM получает фокус-инструкцию и может предлагать **полные имена ключей** `config.env`, в т.ч. пер-тикерные `GAME_5M_TAKE_PROFIT_PCT_<T>` (см. `docs/TRADE_EFFECTIVENESS_ANALYZER.md`).
3. **Осторожное применение чисел из отчёта:** **`scripts/analyzer_tune_apply.py`** — только ключи из белого списка редактируемых (`is_editable_config_env_key`); **пер-тикерный `GAME_5M_STALE_REVERSAL_MIN_AGE_MINUTES_<T>`** в этот список по префиксу **пока не развёрнут** — его можно задать **вручную** в `config.env` или расширить список префиксов в `config_loader`, если решите вести его через веб.

**Итог:** дифференциация «для одних тикеров терпим дольше, для других — как сейчас» **частично уже возможна** (`MIN_AGE` по тикеру, дни и тейк по тикеру); **полный автоматический «советчик» только по stale для всех суффиксов в UI** — пока **нет**, опора на **анализатор + оператор** (и при необходимости расширение whitelist в коде).

---

## 3. Параметры текущего игрового блока

Ключевые параметры берутся из `config.env` (на сервере/в контейнере) через `config_loader.get_config_value()`. Локальную копию часто держат как `config.env.lse` — **не в git**, только у админа. Если один ключ встречается в файле несколько раз, активным будет последнее прочитанное значение.

Пример типичного блока GAME_5M в `config.env` (иллюстрация, не обязательно совпадает с продом):
...
Смысл: если позиция уже достаточно долго в просадке, импульс не поддерживает удержание, а текущая карточка не даёт BUY, позицию можно закрыть раньше обычного срока. В конфигурации по умолчанию этот режим часто выключен (`false`).
