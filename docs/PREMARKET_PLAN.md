...
**Геополитический/премаркет-стресс (Этап 5.2):** если в config заданы `PREMARKET_STRESS_TICKERS` (напр. `GBPUSD=X,EURUSD=X`), крон дополнительно смотрит гэп по этим тикерам. При гэпе вниз сильнее порога `PREMARKET_STRESS_GAP_PCT` (по умолчанию −1.5%) в Telegram уходит второй алерт: «Рекомендуется закрыть позиции GAME_5m и рассмотреть закрытие прочих до открытия сессии». В пятницу в текст добавляется напоминание закрыть позиции до выходных. Так по премаркету Forex можно заранее заметить сильный провал на открытии и по возможности закрыть позиции (GAME_5m и др.) ещё в пятницу. Опция `PREMARKET_STRESS_ALERT_FRIDAY_ONLY=true` — стресс-алерт только в пятницу; `false` — в любой день при срабатывании порога.
