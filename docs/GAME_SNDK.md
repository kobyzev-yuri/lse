...
2. **trade_history**: обычные строки BUY/SELL с `strategy_name='GAME_5M'`, `ticker='SNDK'`. PnL по закрытым парам считаем по лог-доходности (минус комиссия). Портфель и история в одной схеме; фильтр по strategy_name отделяет игру от ручных/других стратегий. Уведомления в Telegram по 5m шлёт только этот скрипт; сделки портфельной игры (другие стратегии) уведомляются из `trading_cycle_cron.py` в те же чаты (`TELEGRAM_SIGNAL_CHAT_IDS`).
