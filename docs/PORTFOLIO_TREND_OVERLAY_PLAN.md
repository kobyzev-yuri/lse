...
|----------|----------|--------|------|
| **Стратегический (~5–6м)** | графики / `ret≈126d` + сейчас 20d | **раз в месяц** (`run_portfolio_prospect_monthly_review.py`, cron 1-е 07:20 MSK) | кого держать в фокусе портфельной игры |
| **Тактический (~20d)** | CatBoost 20d + rule regime | nightly + apply gate | prefer / allow / avoid на каждый BUY |

Графики за полгода — верный вывод о *структурной* перспективности (melt-up names vs late-chase/crash). Рынок меняется → **ежемесячный пересмотр** bucket'ов (`core_prospect` / `watch_long` / `tactical_avoid` / `structurally_weak`), не только nightly score.
