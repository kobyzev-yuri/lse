...
**Как смотреть влияние на игру:** analyzer `portfolio_trend_regime_review` + BUY `context_json.portfolio_ml_20d_*` / `portfolio_prospect_*`.

### Два горизонта перспективности

| Горизонт | Источник | Каденс | Роль |
|----------|----------|--------|------|
| **Стратегический (~5–6м)** | path metrics + capture band + 20d | **раз в месяц** (`run_portfolio_prospect_monthly_review.py`, cron 1-е 07:20 MSK) | куда ставить капитал/внимание первым и какой порядок прибыли |
| **Тактический (~20d)** | CatBoost 20d + rule regime | nightly + apply gate | prefer / allow / avoid на каждый BUY |

**UI:** `/portfolio/daily` — блок «Куда инвестировать в первую очередь» **над** набором графиков (JSON `last_portfolio_prospect_monthly_review.json`, API `GET /api/portfolio/prospect-monthly`). Колонки: rank / bucket / рынок ~6м / capture band / до 6м-хая / exp 20d / действие.

**UI форма-кластеры (Настя):** `/portfolio/shape-clusters` — список ~40 тикеров, кластеры по corr нормированной цены за lookback, навигация по группе: оверлей формы + daily charts. API: `GET /api/portfolio/shape-clusters`, `.../charts`. Позже: log-ret / сектор / earnings как `mode`.

Логика: `services/portfolio_prospect_monthly.py` — path_tag, MFE→capture proxy, `allocation_rank`. Near-6m-high + сильный path → `watch_long` (не core chase). Рынок меняется → **ежемесячный пересмотр**, не nightly score.

---

## Предложения улучшения: portfolio дневка (5d) — **FROZEN (2026-07-17)**

| # | Изменение | Вердикт |
|---|-----------|--------|
| **D1–D4** | drop price / IC gate / FX out / rescale score | **Не ждать.** Live 5d бесполезен (IC≈0, block thr=42 = 0 срезов за 30д). |
| **Дальше** | policy: live exit regime, late-chase CF, peer rank, options | [PORTFOLIO_POLICY_NEXT_PLAN.md](PORTFOLIO_POLICY_NEXT_PLAN.md) |

Не поднимать 5d/20d return-CatBoost ради «оживления»; nightly train можно оставить как telemetry.
...
# Deploy
ssh ai8049520@35.193.21.44 "cd /home/ai8049520/lse && ./scripts/deploy_from_github.sh"
