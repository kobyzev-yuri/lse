...
- \(\text{RSI}_{5m}\) — RSI по 5m закрытиям.
- \(\sigma_{5m}\) — волатильность в %: стандартное отклонение лог-доходностей 5m \(\times 100\) (поле `volatility_5m_pct`).
- \(\text{mom}_{2h}\) — импульс в % (поле `momentum_2h_pct`).
- \(\text{low}_{recent}\) — минимум последних N баров (поле `recent_bars_low_min`).
- \(\text{phase}\) — фаза сессии (`session_phase`: REGULAR, PRE_MARKET и т.п.).
...
2. В `get_decision_5m()`:
   ...
   - `estimated_upside_pct_day = take_pct`  
     (поле **Апсайд (тейк)** в карточке),
   - если \(p > 0\), то:
     \[
       \text{suggested\_take\_price} = p \cdot (1 + \text{take\_pct} / 100)
     \]
     — поле **Цель** в карточке.

Иными словами: апсайд = эффективный интрадей-тейк в процентах, цель = текущая цена \(\times\) (1 + тейк).
...
1. База по волатильности и локальному минимуму:
   ...
   - \(\sigma_{5m} = \text{volatility_5m_pct}\),
   - \(\text{low}_{recent} = \text{recent\_bars\_low\_min}\).

   Если \(\text{low}_{recent} < p\), вычисляем:
   \[
     d_{\text{recent\_low}} = \frac{p - \text{low}_{recent}}{p} \cdot 100
   \]

   Базовый риск:
   \[
     \text{base} = \max\bigl(2.5 \cdot \sigma_{5m},\ d_{\text{recent\_low}}\bigr)
   \]
   (если данные есть; иначе \(\text{base}\) стартует с 0 и дальше может быть скорректирован).

2. Коррекция по импульсу, фазе и RSI:

   - Если \(\text{mom}_{2h} < -1\%\):  
     \(\text{base} \mathrel{*}= 1.2\) (отрицательный импульс усиливает риск).
   - Если \(\text{phase} \in \{\text{PRE\_MARKET}, \text{AFTER\_HOURS}\}\):  
     \(\text{base} \mathrel{*}= 1.1\) (вне основной сессии ликвидность ниже).
   - Если \(\text{RSI}_{5m} \le 25\) (сильная перепроданность):  
     \(\text{base} \mathrel{*}= 0.85\) (downside обычно меньше).
   - Если \(\text{RSI}_{5m} \ge 75\) (перекупленность):  
     \(\text{base} \mathrel{*}= 1.15\).

3. Ограничения и итог:

   - минимальный риск: **0.2%**, максимальный: **25%**:
     \[
       \text{estimated\_downside\_pct\_day} =
       \begin{cases}
         \text{round}(\min(\max(\text{base}, 0.2), 25.0), 2), & \text{если base > 0} \\
         \text{None}, & \text{иначе}
       \end{cases}
     \]
...
1. Начальные веса:
   \[
     \text{up\_score} = 1.0,\quad \text{down\_score} = 1.0
   \]

2. Коррекция по RSI:

   - Если \(\text{RSI}_{5m} \le 30\):  
     \(\text{up\_score} \mathrel{+}= 0.6\)
   - Если \(\text{RSI}_{5m} \ge 70\):  
     \(\text{down\_score} \mathrel{+}= 0.6\)

3. Коррекция по импульсу:

   - Если \(\text{mom}_{2h} \ge 1.0\%\):  
     \(\text{up\_score} \mathrel{+}= 0.4\)
   - Если \(\text{mom}_{2h} \le -1.0\%\):  
     \(\text{down\_score} \mathrel{+}= 0.4\)

4. Коррекция по высокой волатильности:

   - Если \(\sigma_{5m} \ge 0.8\%\), оба отклонения от 1.0 «подтягиваются к 1» (делаются менее резкими). В коде:  
     \[
       \text{up\_score} = 1.0 + (\text{up\_score} - 1.0) \cdot 0.7
     \]
     аналогично для `down_score`.

5. Нормировка до вероятностей:

   \[
     s = \text{up\_score} + \text{down\_score},\quad
     \text{prob\_up} = \frac{\text{up\_score}}{s},\quad
     \text{prob\_down} = \frac{\text{down\_score}}{s}
   \]
   В карточке значения округляются до двух знаков.

Практический смысл: при грубом скане кластера видеть, где \(\text{P(up)}\) явно перевешивает (низкий RSI + положительный импульс) и где риск больше в сторону падения (высокий RSI + отрицательный импульс).
