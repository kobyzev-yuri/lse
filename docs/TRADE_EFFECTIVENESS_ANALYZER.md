...
- **`scripts/snapshot_analyzer_report.py`** — тот же расчёт, что API, **без HTTP** (нужны БД и `PYTHONPATH` как у остальных скриптов). Пишет `analyzer_{STRATEGY}_{N}d_{UTC}.json` и обновляет **`latest.json`** в каталоге.
  - Каталог: **`ANALYZER_SNAPSHOT_DIR`** или по умолчанию `logs/analyzer_snapshots/` в корне репо.
  ...
  - Пример: `ANALYZER_SNAPSHOT_DIR=/var/lib/lse/analyzer_snapshots python3 scripts/snapshot_analyzer_report.py --days 7`
...
Рекомендуемый цикл: снимок → (опционально LLM отдельно) → ручной или скриптовый выбор одного изменения → `analyzer_tune_apply.py --index N` → перезапуск сервиса по вашему `RESTART_CMD` → через N дней новый снимок → `diff_analyzer_snapshots.py`.
