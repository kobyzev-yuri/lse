...
**Операционный чеклист после каждой сессии:** [GAME_5M_SESSION_ANALYSIS_PLAYBOOK.md](GAME_5M_SESSION_ANALYSIS_PLAYBOOK.md).
...
Песочница правил (макро VIX/Forex, прогноз гэпа сектора и т.д.): по `macro_*` в `context_json` на входе сравнивается **средний realized_pct** по корзинам AVOID vs ALLOW и др. Вердикты **keep / caution / remove / insufficient_data** — см. `docs/GAME_5M_PRODUCT_IDEAS_ARBITER.md`. Config **не** меняется автоматически.
...
**Источник 5m OHLC в анализаторе (не путать с live-игрой).** Live `get_decision_5m` использует **`fetch_5m_ohlc` (yfinance)**. Для постфактума `trade_effectiveness_analyzer` кэш 5m по умолчанию выровнен с игрой: **`TRADE_ANALYZER_5M_OHLC_SOURCE=yfinance`** (см. `config.env.example`); значение **`postgres_first`** включает прежний режим «сначала `market_bars_5m` в Postgres». Для `TIME_EXIT_EARLY` при разрыве между `exit_ts` и рядом Yahoo контрфакт якорится на ближайший доступимый 5m-бар; поле **`post_exit_anchor_mode`** в JSON отчёта. Порог **`TIME_EXIT_EARLY_MIN_ROWS_ML`** задаёт, когда `insufficient_data_for_ml` остаётся true при малом числе строк.

- **`scripts/run_analyzer_docker.sh`** — на **хосте VM с Docker** (не внутри образа как единственный способ): вызывает `docker exec lse-bot … export_analyzer_report.py`, по умолчанию 2 дня, LLM, JSON на хост (часто `/tmp/`). Удобно для разового отчёта после деплоя.
