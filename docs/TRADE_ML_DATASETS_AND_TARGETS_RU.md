# Четыре CatBoost-сетки и ridge multiday: 5m вход, portfolio, удержание, «календарь», дневка 1–3 дня

В репозитории **`lse`** четыре обучаемых **CatBoost**-модели с разными юнитами наблюдения, признаками и типом предикта; отдельно — **ridge по дневным лог-доходностям** на 1–3 торговых дня (не CatBoost). Ниже — цель каждой сетки, откуда накапливаются факты и как задаётся **положительный / отрицательный** (или знак) итог для обучения.
...
| **Multiday ridge (дневка, GAME_5M)** | `scripts/train_game5m_multiday_lr.py` | JSON в `GAME_5M_MULTIDAY_LR_MODEL_DIR` (офлайн); онлайн — fit без `.cbm` | регрессия (ridge) |
...
## 5. Multiday ridge (дневка, 1–3 торговых дня, GAME_5M)

Отдельный контур: **ridge-регрессия** по дневным close (и опц. премаркет из БД), горизонты **1 / 2 / 3** торговых дня в log-доходности. Скрипт: `scripts/train_game5m_multiday_lr.py`. Полное описание, env и **фазы калибровки**: [GAME_5M_MULTIDAY_LR_RIDGE.md](GAME_5M_MULTIDAY_LR_RIDGE.md).
...
| **Multiday ridge** | На **конец дня i** по дневному ряду какова **суммарная log-доходность** на **1, 2 и 3** следующих торговых дня (справочно для 5m-карточек; по умолчанию выкл.)? |
