# Группы тикеров: быстрая игра, средние дистанции, вдолгую

## Назначение

Тикеры из `/tickers` (зарегистрированные в БД) распределены по трём группам по стилю торговли:

| Группа | Стиль | Тикеры (по умолчанию) | Логика |
|--------|--------|------------------------|--------|
| **Быстрая игра** | Интрадей, 5m | SNDK, MU, LITE | Короткие интервалы (день–два), 5m индикаторы, GAME_5m (send_sndk_signal_cron). Нужны 5m данные Yahoo. |
| **Средние дистанции** | Смешанный вариант | ALAB, TER, AMD, ANET, INTC, DELL, AVGO, ORCL, PLTR | Среднесрочный горизонт, KB/прогнозы; портфельная игра при `TRADING_CYCLE_ENABLED`. Не путать с FAST 5m. |
| **Вдолгую** | Свинг, дневные | MSFT, META, GOOGL, AMZN, NVDA, ANET, INTC, DELL, ALAB, AVGO, ORCL, PLTR, AMD, SNDK, GBPUSD=X, GC=F, ^VIX, CL=F, BZ=F | Мегакап + драйверы (в т.ч. список шефа + SNDK) + макро/гео; дневные решения и новости. |

## Конфигурация

В `config.env` задайте списки под ваши зарегистрированные инструменты (как в `/tickers`):

```bash
# Быстрая игра (5m) — только тикеры с 5m данными
TICKERS_FAST=SNDK,MU,LITE

# Средние дистанции (драйверы / полупроводники — KB, не обязательно 5m игра)
TICKERS_MEDIUM=ALAB,TER,AMD,ANET,INTC,DELL,AVGO,ORCL,PLTR

# Вдолгую (свинг + мегакап + драйверы + макро)
TICKERS_LONG=MSFT,META,GOOGL,AMZN,NVDA,ANET,INTC,DELL,ALAB,AVGO,ORCL,PLTR,AMD,SNDK,GBPUSD=X,GC=F,^VIX,CL=F,BZ=F

# Портфель: без сделок по металлам/нефти (только корреляция). Пусто = ^VIX,GC=F,CL=F,BZ=F
TICKERS_INDICATOR_ONLY=^VIX,GC=F,CL=F,BZ=F
```

Если не задано — используются дефолты из `services/ticker_groups.py` (см. выше).

### Игра 5m: волатильность и выбор тикеров

Стратегия GAME_5m рассчитана на **волатильные** интрадей движения: тейк +5%, стоп −2.5% и т.п. имеют смысл, когда цена за сессию заметно ходит.

- **GC=F (золото)** — дневной/свинг актив; у него нет смысла в **быстрой игре 5m** (интрадей по 5m свечам). Держите GC=F только в `TICKERS_LONG`; покупка вручную: `/buy GC=F 1`. Если добавить GC=F в `TICKERS_FAST`/`GAME_5M_TICKERS`, крон будет пытаться обрабатывать его как 5m и возможны ошибки рассылки (HTTP 400) и лишние логи «уже в позиции».
- **LITE** по наблюдениям ведёт себя как **низковолатильная** бумага: цена часто держится в узком коридоре (например 670–720) за день, движения небольшие. Входы/выходы по той же логике, что и для SNDK, дают мало профита и легко уходят в минус (закрытие по SELL при небольшом движении). **Играть с LITE как с SNDK смысла мало.**

Чтобы оставить LITE в графиках и рекомендациях 5m, но **не запускать по нему игру** (вход/выход кроном), задайте в `config.env`:

```bash
GAME_5M_TICKERS=SNDK,MU
```

Тогда `send_sndk_signal_cron` будет открывать/закрывать позиции только по SNDK и MU; LITE остаётся в `TICKERS_FAST` для `/chart5m`, `/recommend5m` и т.д. Если `GAME_5M_TICKERS` не задан, используются все тикеры из `TICKERS_FAST`.

## Условие для быстрых тикеров: наличие 5m данных

Для быстрой игры и рассылки сигналов нужны **5-минутные котировки** (как для `/chart5m`). Источник — Yahoo Finance; не у всех тикеров есть 5m.

- **Cron** `send_sndk_signal_cron.py` обрабатывает только те тикеры из `TICKERS_FAST`, по которым **есть 5m данные** (проверка `has_5m_data()` в `services/recommend_5m.py`). Остальные в этом запуске пропускаются с предупреждением в логе.
- **Проверка вручную:**  
  `python scripts/check_fast_tickers_5m.py` — выводит, у каких быстрых тикеров есть 5m, у каких нет. Тикеры без 5m лучше убрать из `TICKERS_FAST`, пока нет другого источника интрадей-данных.

## Использование в коде

```python
from services.ticker_groups import get_tickers_fast, get_tickers_medium, get_tickers_long, get_all_ticker_groups

fast = get_tickers_fast()     # из TICKERS_FAST, напр. ["SNDK", "MU", "LITE"]
medium = get_tickers_medium() # из TICKERS_MEDIUM, напр. ["ALAB", "MU", "TER"]
long_ = get_tickers_long()    # из TICKERS_LONG, напр. ["MSFT", "GBPUSD=X", "GC=F", "^VIX"]
all_ = get_all_ticker_groups()  # быстрые → средние → долгие, без дубликатов
```

Дашборд, рекомендации 5m, game_* и стратегии могут опираться на эти списки (сейчас дашборд использует `DASHBOARD_WATCHLIST`; при расширении можно собирать watchlist из `get_all_ticker_groups()` или разделять блоки «быстрые» / «средние» / «долгие»).

## Как добавить новый тикер (например AMD)

Чтобы новый тикер участвовал в **учёте цен** и **новостях**:

### 1. Цены (таблица `quotes`, `/tickers`, обновление по крону)

- **Один раз** загрузить котировки для тикера:
  ```bash
  cd /path/to/lse && python update_prices.py AMD
  ```
  После этого тикер появится в `quotes`, в `/tickers` и будет обновляться кроном `update_prices_cron.py` вместе с остальными.

- Либо добавить тикер в `DEFAULT_TICKERS` в `init_db.py` и при следующем `python init_db.py` (или ручном вызове `seed_data()`) он подтянется при первичной загрузке.

### 2. Группа (игры и список для новостей)

В `config.env` добавьте тикер в нужную группу:

- **Быстрая игра (5m):** `TICKERS_FAST=SNDK,MU,LITE,AMD` — только если по нему есть 5m данные (Yahoo).
- **Средние:** `TICKERS_MEDIUM=ALAB,MU,TER,AMD`
- **Вдолгую:** `TICKERS_LONG=MSFT,GBPUSD=X,GC=F,^VIX,AMD`

От группы зависят: GAME_5m (только FAST), портфельная игра (MEDIUM + LONG или `TRADING_CYCLE_TICKERS`).

### 3. Новости

- **Investing.com News** — тикеры берутся из **всех групп** (FAST + MEDIUM + LONG). Достаточно добавить AMD в одну из переменных выше; при необходимости добавьте ключевые слова в `config.env`:
  ```bash
  INVESTING_NEWS_TICKER_KEYWORDS=AMD:Advanced Micro Devices
  ```
  (для части тикеров ключевые слова уже заданы в коде в `services/investing_news_fetcher.py`.)

- **Alpha Vantage:** в `config.env` добавьте тикер в список для новостей:
  ```bash
  EARNINGS_TRACK_TICKERS=MSFT,SNDK,MU,LITE,ALAB,TER,AMD,NVDA
  ```
  Если `EARNINGS_TRACK_TICKERS` задан — Yahoo earnings calendar всё равно **добавляет union** из `get_earnings_intelligence_universe()` (NVDA, DELL, GOOGL, …). Полный дефолт: `DEFAULT_EARNINGS_TRACK_TICKERS` в `services/earnings_intelligence_universe.py`.

- **LLM-новости** (если используете): добавьте в
  ```bash
  LLM_NEWS_TICKERS=SNDK,AMD
  ```

### Кратко для AMD

1. `python update_prices.py AMD` — один раз для цен.
2. В `config.env`: добавить `AMD` в `TICKERS_MEDIUM` (или в `TICKERS_LONG`).
3. При необходимости: добавить `AMD` в `EARNINGS_TRACK_TICKERS` и/или `LLM_NEWS_TICKERS`.

После этого AMD будет в `/tickers`, в портфельной игре (если в MEDIUM/LONG) и в новостях (Investing.com по группе, Alpha Vantage и LLM — по конфигу).

## Нефть (CL=F) — индикатор геополитики, торговля, новости

**CL=F** (WTI crude oil futures) добавлен в дефолт **TICKERS_LONG** и используется как:

- **Индикатор геополитической напряжённости** — рост нефти часто совпадает с эскалацией (Ближний Восток, санкции). Можно добавить в `PREMARKET_STRESS_TICKERS` для мониторинга резких движений.
- **Торговля** — портфельная игра (trading_cycle_cron) по списку MEDIUM + LONG; стратегии AnalystAgent применяются к CL=F как к прочим тикерам.
- **Новости** — тикер входит в `get_tracked_tickers_for_kb()` (через TICKERS_LONG), поэтому новости по нефти сохраняются в knowledge_base и участвуют в анализе (в т.ч. для других инструментов при запросе MACRO/US_MACRO или по тикеру).

Переопределить тикер нефти (например на BZ=F — Brent): в `config.env` задать `OIL_TICKER=BZ=F`. В коде: `from services.ticker_groups import get_oil_ticker`.

После добавления CL=F в конфиг один раз загрузите котировки: `python update_prices.py CL=F`.

## Геополитика: падение и отскок

При геополитических шоках (военные угрозы, эскалация) рынок часто сначала резко падает (реализация страха), затем даёт **отскок** в течение 1–3 сессий. Long на отскоке часто даёт хорошую точку входа.

### Формализованное правило

- **Триггер:** резкое падение индекса/тикера на **≥ 2% за одну сессию** (дневной candle) на фоне геополитической новости (без явного «разруливания»).
- **Действие:** не уходить сразу в кэш; допускать **вход в long на первом отскоке** (первая сессия с закрытием выше минимума паники или следующая зелёная свеча).
- **Риск-менеджмент:** размер позиции — в рамках обычных лимитов; **стоп** — под локальный минимум паники (или под минимум отскока, если он уже сформировался).
- **Область применения:** тикеры из «вдолгую» и «средние» (индексы, акции, GC=F и т.д.); для быстрой игры (5m) правило не формализовано.

Параметры (порог падения %, окно отскока в сессиях) при необходимости выносятся в конфиг или в настройки стратегии. Реализация в коде: **стратегия `Geopolitical Bounce`** в `strategies/geopolitical_bounce_strategy.py`; при выборе стратегии в `StrategyManager` она проверяется первой при падении предыдущей сессии ≥2%.

### Контекст: новости (пример)

> **US military prepared to strike Iran as early as this weekend, but Trump has yet to make a final call, sources say**  
> (Источники: CNN, Reuters и др., февраль 2026.)

При таких заголовках типична волатильность и возможность отскока после первого падения; решение о входе в long на отскоке принимается вручную или будущей стратегией с учётом стоп-лосса.

## См. также

- [earnings-event-agent-lse/PUBLIC_IR_EARNINGS_SOURCES.md](earnings-event-agent-lse/PUBLIC_IR_EARNINGS_SOURCES.md) — официальные IR и квартальная отчётность: дефолтные акции из этой таблицы групп + ASML, ARM; примеры кварталов
- [GAME_5M_CALCULATIONS_AND_REPORTING.md](GAME_5M_CALCULATIONS_AND_REPORTING.md) — игра по 5m, быстрые тикеры
- [TELEGRAM_BOT_SETUP.md](TELEGRAM_BOT_SETUP.md) — дашборд, рассылка сигналов
- [RISK_MANAGEMENT.md](RISK_MANAGEMENT.md) — бюджет и лимиты компании для игры
