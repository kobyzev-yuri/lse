...
Архивный снимок отчёта для шефа за 2026-04-18 сохранён как `docs/archive/GAME5M_TAKE_5M_VS_30M_EXECUTIVE_REPORT_2026-04-26.md`; свежие таблицы нужно генерировать заново из JSON по командам выше.

---

## 7. «Висяк» для подбора выхода (интуиция и задел под скрипт)

**Интуитивно:** позиция **4–6 дней** не получает **`TAKE_PROFIT`** (порог по правилам GAME_5M так и не выполнен на закрытиях баров), при этом за тот же период по бумаге видна **тенденция к провисанию** — цена не наращивает уверенный хвост к тейку, а уходит в бок/вниз относительно входа.

**Для кода (все условия одновременно):**

1. **Время без тейка:** на горизонте **D** календарных дней после входа (D из диапазона 4…6, фиксируется параметром) по реплею на 5m-барах ни на одном баре до конца окна не сработал выход с `signal_type == TAKE_PROFIT` (в т.ч. не сработал «мягкий» тейк у хая, если он включён).
2. **Провисание рынка:** на том же окне `[entry … entry+D]` выполняется хотя бы один количественный признак (выбирается один вариант при реализации и не меняется между прогонами):
   - **Вариант A (простой):** \(\ln(\text{close}_{\text{конец}} / \text{entry}) \le \varepsilon\) с небольшим \(\varepsilon\) (например 0 или −0,005) — к концу окна нет устойчивого плюса от входа;
   - **Вариант B (high-water):** \(\max_t(\text{high}_t) / \text{entry} - 1\) не достигал бы **эффективного** порога тейка в процентах, рассчитанного как в проде на соответствующих барах — «к рынку тянулись», но не дотянули;
   - **Вариант C (наклон):** линейный тренд по `Close` (или log-close) по барам окна **≤ 0** — явное слабое скольжение.

Так отсекаются ситуации «просто долго ждём тейк при растущем рынке»: без сочетания с **ослаблением** цены/тренда это не висяк в смысле подбора «спасательных» параметров.

*Примечание:* календарь vs только RTH-сессии задаётся в скрипте одним флагом; для согласования с бэктестом лучше явно писать в отчёте прогона, какой вариант взят.
