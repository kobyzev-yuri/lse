...
- **Премаркет-крон:** 16:30 MSK пн–пт → сбор премаркета по тикерам → лог и опционально алерт в Telegram. При заданных `PREMARKET_STRESS_TICKERS` (напр. GBPUSD=X, EURUSD=X) при сильном гэпе вниз — доп. алерт «закрыть GAME_5m и прочие до открытия» (в пятницу — до выходных), см. [PREMARKET_PLAN.md](PREMARKET_PLAN.md).
