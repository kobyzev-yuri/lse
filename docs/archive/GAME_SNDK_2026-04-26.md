...
- **Премаркет:** при фазе PRE_MARKET решение обогащается данными премаркета (цена, гэп к закрытию, минуты до открытия); в ответе — рекомендация по входу (войти по текущей цене / ждать 9:30 ET / лимит ниже). Вход в позицию отложен до открытия биржи. Подробнее: [docs/PREMARKET_PLAN.md](PREMARKET_PLAN.md); историческое резюме — [docs/archive/RESUME_PREMARKET_AND_RECENT.md](../archive/RESUME_PREMARKET_AND_RECENT.md).

## BUY и STRONG_BUY: в чём разница

- **STRONG_BUY** — более сильный сигнал: высокая уверенность (например, явная перепроданность RSI(5m) ≤ 32 и отскок, или высокий confidence от стратегий/LLM). В правилах 5m (`recommend_5m.py`) STRONG_BUY выставляется при жёстких условиях: RSI ≤ 32 и импульс 2ч ≥ −0,3%.
- **BUY** — более осторожный сигнал: вход допустим, но уверенность ниже (например, RSI ≤ 38 у 5д минимума или импульс + RSI без перекупленности; в MeanReversionStrategy — при confidence ≤ 0,75).

**Влияние на решение (единственное место, где они обрабатываются по-разному):** учёт новостей из KB в `get_decision_5m()`:
- **Сильный негатив** (sentiment < 0,35): и STRONG_BUY, и BUY переводятся в **HOLD** (вход отложен).
- **Негатив** (0,35–0,4): STRONG_BUY **ослабляется до BUY** (вход разрешён, но без «сильного» сигнала); BUY остаётся BUY (добавляется предупреждение «осторожность»).
- **Позитив** и остальные случаи: оба сигнала не меняются.

**На исполнение и рассылку** различие не влияет: и BUY, и STRONG_BUY приводят к одному и тому же действию — открытие позиции кроном, запись BUY в trade_history и отправка уведомления в Telegram. Размер позиции и условия тейка/стопа не зависят от того, был ли сигнал BUY или STRONG_BUY.
...
2. **trade_history**: обычные строки BUY/SELL с `strategy_name='GAME_5M'`, `ticker='SNDK'`. Метка времени **`ts`** в БД хранится в **московском времени** (колонка **`ts_timezone`** по умолчанию `Europe/Moscow`); при сравнении с 5m-барами и отображении используется перевод в **ET** (см. [TIMEZONES.md](TIMEZONES.md)). PnL по закрытым парам считаем по лог-доходности (минус комиссия). Портфель и история в одной схеме; фильтр по strategy_name отделяет игру от ручных/других стратегий. Уведомления в Telegram по 5m шлёт только этот скрипт; сделки портфельной игры (другие стратегии) уведомляются из `trading_cycle_cron.py` в те же чаты (`TELEGRAM_SIGNAL_CHAT_IDS`).
...
4. **Параметры стратегии** задаются в `config.env` (по умолчанию — как ниже). **Видны везде в мониторинге** — ими удобно варьировать:
   - **Стоп 5m:** включён/выключен по **GAME_5M_STOP_LOSS_ENABLED** (true/false). При false закрытие только по тейку/TIME_EXIT/SELL; в ответах LLM не рекомендуем стоп по 5m. При true: стоп = min(`GAME_5M_STOP_LOSS_PCT`, эффективный_тейк × `GAME_5M_STOP_TO_TAKE_RATIO`), не меньше `GAME_5M_STOP_LOSS_MIN_PCT`.
   - `GAME_5M_STOP_LOSS_PCT=2.5` — верхняя граница стопа в % (если стоп включён).
   - `GAME_5M_STOP_TO_TAKE_RATIO=0.5` — стоп не больше половины тейка (0.5 = 50%).
   - `GAME_5M_STOP_LOSS_MIN_PCT=0.5` — минимальный стоп в %.
   - **Тейк-профит по импульсу:** импульс 2ч (как у модели). Импульсы **менее** `GAME_5M_TAKE_PROFIT_MIN_PCT` не рассматриваем — тейк берётся из конфига; если импульс ≥ порога — тейк от импульса 2ч (Alex: «импульсы менее 2% не рассматриваем»).
   - `GAME_5M_TAKE_PROFIT_PCT=5.0` — тейк по умолчанию (когда импульс < порога или отсутствует).
   - `GAME_5M_TAKE_PROFIT_MIN_PCT=2.0` — минимальный импульс 2ч (%), при котором тейк берётся из импульса; ниже — используем фиксированный тейк.
...
## Тикер «пропал» из игры: как вернуть

В **игре 5m** участвуют только тикеры из **GAME_5M_TICKERS** (или **TICKERS_FAST**, если GAME_5M_TICKERS не задан) **и** только те, по которым в момент запуска крона есть **5m данные** (Yahoo). Если тикер «исчез»:

1. **Проверьте конфиг**  
   В `config.env` должно быть, например:
   ```env
   TICKERS_FAST=SNDK,MU
   GAME_5M_TICKERS=SNDK,MU
   ```
   Без запятой, без лишних пробелов. Если задан только `GAME_5M_TICKERS=MU`, то SNDK в игру не попадает — добавьте SNDK в список.

2. **Проверьте логи крона**  
   При запуске `send_sndk_signal_cron.py` в логе будет строка вида:
   - `Игра 5m: тикеры из конфига ['SNDK', 'MU'] → обрабатываем ['SNDK', 'MU']` — оба в игре.
   - `Игра 5m: тикеры из конфига ['SNDK', 'MU'] → обрабатываем ['MU']` и предупреждение `SNDK: нет 5m данных (Yahoo), пропуск` — SNDK выкидывается из этого запуска, потому что **Yahoo не вернул 5m данные** (выходные, праздник, сбой или лимит API). В следующем запуске в часы торгов данные обычно появляются — тикер снова начнёт обрабатываться без смены конфига.

3. **В /pending «5m вне»**  
   Если по открытой позиции показывается стратегия **«5m вне»**, значит тикер сейчас **не входит** в `get_tickers_game_5m()` (его убрали из GAME_5M_TICKERS или TICKERS_FAST). Чтобы снова вести эту позицию из крона (тейк/стоп/SELL), верните тикер в **GAME_5M_TICKERS** (или в TICKERS_FAST, если GAME_5M_TICKERS не задан) и перезапускать ничего не нужно — следующий запуск крона подхватит конфиг.

Итог: «вернуть SNDK в игру» = убедиться, что он есть в **GAME_5M_TICKERS** (или TICKERS_FAST) и что крон запускается в часы торгов, когда по нему есть 5m данные.
...
**4. Как исправить пропущенный тейк сейчас (закрыть позицию вручную)**

Если позиция GAME_5M осталась открытой, хотя цена достигала тейка, можно закрыть её вручную с типом выхода TAKE_PROFIT (текущая цена берётся из `quotes`):

```bash
# Закрыть по текущей цене как TAKE_PROFIT (по умолчанию)
python scripts/close_game_position.py SNDK

# Или явно указать тип: TAKE_PROFIT | STOP_LOSS | MANUAL | SELL | TIME_EXIT
python scripts/close_game_position.py SNDK TAKE_PROFIT
```

Скрипт проверит наличие открытой позиции по тикеру, возьмёт последнюю цену из `quotes` и запишет SELL в `trade_history` с `strategy_name='GAME_5M'` и указанным `signal_type`. После этого позиция считается закрытой, PnL попадёт в отчёт по сделкам.

**5. Что смотреть и что менять для улучшения**
