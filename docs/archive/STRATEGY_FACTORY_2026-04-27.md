...
- Экстремальный sentiment (>0.6 по модулю)
...
#### 4. NeutralStrategy (Нейтральный режим)

**Когда используется:**
- Только как fallback: когда ни одна из стратегий выше не подходит по условиям (нет явного тренда, гэпа или экстремального sentiment).
- Не выбирается по `is_suitable()` — вызывается явно из Strategy Manager.

**Характеристики:**
- Сигнал: консервативный **HOLD**
- Рекомендация: удержание до появления чётких условий для другой стратегии
- Подходит для валютных пар и инструментов в боковике (например, GBPUSD=X при нейтральных данных)
...
print(result['selected_strategy'])  # Momentum, Mean Reversion, Volatile Gap, Neutral
