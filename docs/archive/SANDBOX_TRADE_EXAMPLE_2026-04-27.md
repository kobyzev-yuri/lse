...
- Комиссия: **0%** (`COMMISSION_RATE = 0` — оплаты брокеру нет).
- Цены берутся из таблицы `quotes` (последняя цена закрытия по тикеру).
- **Время в trade_history:** колонка `ts` записывается в **биржевом времени (America/New_York, ET)** — сессия PostgreSQL при подключении из приложения выставляется в ET, чтобы сравнивать с 5m-данными и графиками. В `/history` и отчётах время показывается в ET.

**Ограничение:** песочница не знает, по какой реальной цене исполнится сделка. При продаже цена могла уже просесть — исполнение может быть хуже последней котировки. Чтобы учесть это консервативно, в `config.env` можно задать **SANDBOX_SLIPPAGE_SELL_PCT** (например `0.3` = 0,3%): тогда при продаже (ручной и по стоп-лоссу) цена исполнения считается как `последняя_цена × (1 − slippage% / 100)`. По умолчанию 0 (отключено).
...
- Комиссия: **0** USD (COMMISSION_RATE=0)  
- Списывается с кэша: **1 255.00** USD  

**portfolio_state** после сделки:

| ticker | quantity | avg_entry_price | last_updated        |
|--------|----------|-----------------|---------------------|
| CASH   | 98745.00 | 0               | 2026-02-20 10:15:00 |
...
|----|---------------------|--------|------|----------|--------|------------|-------------|-------------|--------------------|---------------|
| 1  | 2026-02-20 10:15:00 | SNDK   | BUY  | 10       | 125.50 | 0          | MANUAL      | 1255.00     | 0.45               | Manual        |
...
|----|---------------------|--------|------|----------|--------|------------|-------------|-------------|--------------------|---------------|
| 1  | 2026-02-20 10:15:00 | SNDK   | BUY  | 10       | 125.50 | 0          | MANUAL      | 1255.00     | 0.45               | Manual        |
| 2  | 2026-02-20 11:00:00 | GC=F   | BUY  | 2        | 2650.00| 0          | MANUAL      | 5300.00     | 0.62               | Manual        |
...
- Комиссия: **0** USD  
- Зачисляется на кэш: **1 272.00** USD  
- P&L по позиции: (127.20 − 125.50) × 10 = **17.00** USD  
...
| ticker | quantity | avg_entry_price | last_updated        |
|--------|----------|-----------------|---------------------|
| CASH   | 94717.00 | 0               | 2026-02-20 16:30:00 |
| GC=F   | 2        | 2650.00         | 2026-02-20 11:00:00 |
...
| 1  | 2026-02-20 10:15:00 | SNDK   | BUY  | 10       | 125.50 | 0          | MANUAL      | 1255.00     | 0.45               | Manual        |
| 2  | 2026-02-20 11:00:00 | GC=F   | BUY  | 2        | 2650.00| 0          | MANUAL      | 5300.00     | 0.62               | Manual        |
| 3  | 2026-02-20 16:30:00 | SNDK   | SELL | 10       | 127.20 | 0          | MANUAL      | 1272.00     | 0.52               | Manual        |
...
| ticker | quantity | avg_entry_price | last_updated        |
|--------|----------|-----------------|---------------------|
| CASH   | 97357.00 | 0               | 2026-02-20 17:00:00 |
...
|----|---------------------|--------|------|----------|--------|------------|-------------|-------------|--------------------|---------------|
| …  | …                   | …      | …    | …        | …      | …          | …           | …           | …                  | …             |
| 4  | 2026-02-20 17:00:00 | GC=F   | SELL | 1        | 2640.00| 0          | MANUAL      | 2640.00     | 0.58               | Manual        |
...
| ticker | quantity | avg_entry_price | last_updated        |
|--------|----------|-----------------|---------------------|
| CASH   | 99874.50 | 0               | 2026-02-21 09:05:00 |
...
|----|---------------------|--------|------|----------|---------|------------|-------------|-------------|--------------------|------------------|
| …  | …                   | …      | …    | …        | …       | …          | …           | …           | …                  | …                |
| 5  | 2026-02-21 09:05:00 | GC=F   | SELL | 1        | 2517.50 | 0          | STOP_LOSS   | 2517.50     | 0.35               | MomentumStrategy |
