# Объяснение: Какие тикеры используются после установки cron

## Краткий ответ

**ДА, для песочницы после установки cron будут использоваться только MSFT, SNDK, GBPUSD=X**, но с важными оговорками.

---

## Детальное объяснение

### 1. Скрипт обновления цен (`update_prices_cron.py`)

```python
# scripts/update_prices_cron.py
update_all_prices()  # Без параметров
```

**Логика:**
- Вызывает `update_all_prices()` без параметров
- `update_all_prices()` когда `tickers=None`, берет **ВСЕ тикеры из БД** через `get_tracked_tickers(engine)`
- `get_tracked_tickers()` делает `SELECT DISTINCT ticker FROM quotes`

**Изначально в БД:**
- При инициализации через `init_db.py` загружаются только: `["MSFT", "SNDK", "GBPUSD=X"]`
- Эти тикеры попадают в таблицу `quotes`

**После установки cron:**
- ✅ `update_prices_cron.py` будет обновлять только MSFT, SNDK, GBPUSD=X (потому что только они есть в БД)

**НО! Если добавить другие тикеры:**
- Если запустить `python update_prices.py AAPL,TSLA`, то эти тикеры добавятся в `quotes`
- После этого `update_prices_cron.py` начнет обновлять **ВСЕ тикеры** из БД (включая AAPL, TSLA)

---

### 2. Скрипт торгового цикла (`trading_cycle_cron.py`)

```python
# scripts/trading_cycle_cron.py
DEFAULT_TICKERS = ["MSFT", "SNDK", "GBPUSD=X"]

if len(sys.argv) > 1:
    tickers = [t.strip() for t in sys.argv[1].split(',')]
else:
    tickers = DEFAULT_TICKERS
```

**Логика:**
- Использует **жестко заданный список** `DEFAULT_TICKERS`
- Можно переопределить через аргументы командной строки

**После установки cron:**
- ✅ `trading_cycle_cron.py` будет торговать только MSFT, SNDK, GBPUSD=X
- ❌ Даже если в БД есть другие тикеры, они **НЕ будут** использоваться для торговли

---

## Выводы

### Для песочницы (минимальный набор):

1. **Обновление цен**: Обновляет только те тикеры, которые есть в таблице `quotes`
   - Изначально: MSFT, SNDK, GBPUSD=X
   - Можно добавить другие через `update_prices.py`

2. **Торговый цикл**: Торгует только MSFT, SNDK, GBPUSD=X (жестко задано)
   - Не зависит от того, какие тикеры есть в БД

### Если нужно изменить список тикеров для торговли:

**Вариант 1: Изменить DEFAULT_TICKERS в коде**
```python
# scripts/trading_cycle_cron.py
DEFAULT_TICKERS = ["MSFT", "SNDK", "GBPUSD=X", "AAPL"]  # Добавить новые
```

**Вариант 2: Использовать конфигурационный файл**
```python
# Можно добавить в config.env:
TRADING_TICKERS=MSFT,SNDK,GBPUSD=X,AAPL
```

**Вариант 3: Переопределить через cron**
```bash
# В crontab можно указать аргументы:
0 9,13,17 * * 1-5 cd $PROJECT_DIR && $PYTHON_PATH scripts/trading_cycle_cron.py MSFT,SNDK,GBPUSD=X,AAPL
```

---

## Рекомендации для песочницы

### Минимальная конфигурация (текущая):
- ✅ MSFT - стабильная акция для тестирования Mean Reversion
- ✅ SNDK - волатильная акция для тестирования Momentum/VolatileGap
- ✅ GBPUSD=X - валютная пара для анализа FX impact

### Если нужно добавить больше тикеров:

1. **Добавить в БД для обновления цен:**
   ```bash
   python update_prices.py AAPL,TSLA,GOOGL
   ```

2. **Добавить в торговый цикл:**
   - Изменить `DEFAULT_TICKERS` в `trading_cycle_cron.py`
   - Или использовать конфигурационный файл

3. **Переустановить cron:**
   ```bash
   ./setup_cron.sh
   ```

---

## Текущее состояние

После установки cron через `./setup_cron.sh`:

- ✅ **Обновление цен** (18:00 ежедневно): MSFT, SNDK, GBPUSD=X
- ✅ **Торговый цикл** (9:00, 13:00, 17:00 пн-пт): MSFT, SNDK, GBPUSD=X

**Это правильно для песочницы!** 🎯

---

## См. также

- [setup_cron.sh](../setup_cron.sh) - скрипт установки cron
- [scripts/update_prices_cron.py](../scripts/update_prices_cron.py) - скрипт обновления цен
- [scripts/trading_cycle_cron.py](../scripts/trading_cycle_cron.py) - скрипт торгового цикла
- [update_prices.py](../update_prices.py) - логика обновления цен
- [init_db.py](../init_db.py) - инициализация БД с начальными тикерами



