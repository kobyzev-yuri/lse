...
Премаркет не выделяется в отдельную торговую игру. Для `GAME_5M` он должен использоваться как контекст основного intraday-решения после открытия: оценка gap continuation / gap fade, риска ложного входа на открытии, вероятности stuck/hanger и вероятности продолжения после первого take.
...
- Premarket нужен не для отдельного BUY до открытия, а для оценки качества входов и выходов в regular-сессию.
