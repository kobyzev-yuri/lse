...
| Блок | Поле(я) | Что писать | Откуда |
|------|---------|------------|--------|
| A | `exchange`, имя/`sym` (read-only), слой из **Профиля**, `hq_ru`, `listing_origin_ru` | Символ+биржа; слой 2–3 слова; город/страна; год + фраза истории | Биржа/IR/6-K; слой — классификация тетрадки |
| B | `tagline`, `key_clients_ru`, `margin_ru` | Модель 1–2 фразы + маржа; клиенты опц. | Business / IR / call |
| C | `metrics[4]` k/v/note/tone | Кэш, FCF, долг, запас — число + цвет + сноска | Баланс / CF; Yahoo draft |
| D | `financing_ru` | Источник капитала, качество долга, точка разрыва | Отчёт + аналитика |
| E | `pluses`, `risks` (+ консенсус read-only) | 2–3 буллета; консенсус с Инвестдомов | Факты блоков выше; StockAnalysis |
