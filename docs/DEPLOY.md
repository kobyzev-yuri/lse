...
Скрипт `scripts/analyzer_autotune.py` предназначен для **эволюционного** автотюнинга параметров `GAME_5M_*`:
один шаг → ожидание накопления новых сделок → следующий шаг.

Рекомендации:
- Запускайте **1 раз в день** или реже (например, после закрытия рынка), чтобы не подкручивать параметры на шуме.
- Держите `ANALYZER_AUTOTUNE_APPLY=0` (dry-run) первые 3–7 дней: проверьте, какие ключи выбираются и что пишет в state.
- Для применения включайте `ANALYZER_AUTOTUNE_APPLY=1` только после того, как уверены в гардрейлах и белом списке ключей.
- После применения параметров нужен **рестарт** сервиса (через `RESTART_CMD` или вручную).

Пример cron (хост, веб доступен локально на 8080):

```bash
30 23 * * 1-5 cd /home/USER/lse && ANALYZER_AUTOTUNE_APPLY=0 \
...
### 5.2. Проверка новостей на сервере (БД в контейнере, запись в knowledge_base)

Если новости не появляются, на сервере выполните по шагам (используем **docker compose**, сервис `lse`):

**1. Диагностика (чтение БД + тест записи + один прогон RSS):**

```bash
cd ~/lse
docker compose exec lse python scripts/check_news_sources.py
```

Скрипт покажет: число записей в `knowledge_base`, последнюю дату, **тест записи в БД (INSERT + ROLLBACK)** — если здесь ошибка, запись из cron тоже будет падать (сеть до PostgreSQL, права, неверный DATABASE_URL). Затем один раз подтянет RSS и выведет «сохранено N новых, дубликатов M».

**2. Полный прогон сбора новостей и лог:**

```bash
docker compose exec lse python scripts/fetch_news_cron.py
tail -80 ~/lse/logs/news_fetch.log
```

В конце лога должна быть строка **«За этот запуск всего сохранено новых записей: N»** и по каждому источнику — сколько сохранено. Если при этом в шаге 1 тест записи прошёл, но N всегда 0 — значит все пришедшие новости уже есть в БД (дубликаты) или источник не отдаёт данные (сеть, 403, лимит API).

**Примечание:** сервер в США; **INVESTING_NEWS_PROXY** (и SOCKS) для новостей Investing.com не нужен — можно не задавать в `config.env`. Прокси имеет смысл только при 403 от Investing.com (например, в другом регионе).
