...
Старый путь **`/trading`** перенаправляет на **`/`** (раздел убран как дублирующий портфель и Telegram). Для ручного запуска портфельного цикла по-прежнему доступны API `POST /api/execute` и крон `trading_cycle_cron.py`.
