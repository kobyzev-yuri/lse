...
# Новости core-fast: каждые 15 мин (RSS + Alpha Vantage)
*/15 * * * * docker exec $CONTAINER_NAME python scripts/fetch_news_cron.py --mode core-fast >> "$PROJECT_DIR/logs/news_fetch.log" 2>&1
# Новости NewsAPI: раз в 2 часа (отдельно, чтобы backoff не тормозил другие источники)
10 */2 * * * docker exec $CONTAINER_NAME python scripts/fetch_news_cron.py --mode newsapi >> "$PROJECT_DIR/logs/news_fetch.log" 2>&1
# Новости Investing: раз в 2 часа (снижение 429)
0 */2 * * * docker exec $CONTAINER_NAME python scripts/fetch_news_cron.py --mode investing >> "$PROJECT_DIR/logs/news_fetch.log" 2>&1
