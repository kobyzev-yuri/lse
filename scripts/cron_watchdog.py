...
Запуск:
  ...
  python scripts/cron_watchdog.py --execute --telegram  # + отправка в Telegram
...
(при CRON_WATCHDOG_TELEGRAM=true тот же запуск отправит алерт в Telegram при находках.)
"""
...
def main():
    ...
    if findings:
        ...
        if use_telegram:
            send_watchdog_to_telegram(findings, logs_dir)
    else:
